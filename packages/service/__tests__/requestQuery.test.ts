...
import { UserService, CourseService, RequestService } from '../lib'
...
describe('RequestQuery', () => {
  ...
  describe('getCourseRequests', () => {
    test('should return correct course requests', async () => {
      const requests = await courseService.getCourseRequests({ code: 'COMP 1023', term: '2510' })
    ...
    test('should return empty array when no requests exist', async () => {
      const requests = await courseService.getCourseRequests({ code: 'COMP 2012', term: '2510' })
